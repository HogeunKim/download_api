...
# ---------------------------------------------
# Ubuntu script: load go-api Docker image tar and run container on user-defined network
# Usage:
#   chmod +x run-docker-net-go-server.sh
#   ./run-docker-net-go-server.sh go-api-server_1.0.1.tar /home/genes007/api
#
# Required argument:
...
if [[ -z "${IMAGE_TAR_INPUT}" ]]; then
  echo "Usage: ./run-docker-net-go-server.sh <IMAGE_TAR> <HOST_OUTPUT_DIR>" >&2
...
echo "[1/6] Ensure output directory"
mkdir -p "${HOST_OUTPUT_DIR_CLEAN}"

echo "[2/6] Ensure docker network exists"
${DOCKER} network inspect "${NETWORK_NAME}" >/dev/null 2>&1 || ${DOCKER} network create "${NETWORK_NAME}" >/dev/null

echo "[3/6] Docker load"
${DOCKER} load -i "${IMAGE_TAR}"

echo "[4/6] Stop old container (if running): ${CONTAINER_NAME}"
${DOCKER} stop "${CONTAINER_NAME}" >/dev/null 2>&1 || true

echo "[5/6] Remove old container (if exists): ${CONTAINER_NAME}"
${DOCKER} rm "${CONTAINER_NAME}" >/dev/null 2>&1 || true

echo "[6/6] Run new container"
${DOCKER} run -d \
  --name "${CONTAINER_NAME}" \
